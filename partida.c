...
// 0 = Piedra, 1 = Papel, 2 = Tijera, 3 = Fuego, 4 = Esponja, 5 = Aire 6 = Agua
const char *nombresElementos[7] = {"Piedra", "Papel", "Tijera", "Fuego", "Esponja", "Aire", "Agua"};
...
static const char *archivosImagenes[7] = {
    "piedra.png", "papel.png", "tijera.png", "fuego.png", "esponja.png", "aire.png", "agua.png"
};

// Texturas ya cargadas en memoria de video, listas para dibujar
static Texture2D imagenesElementos[7];
...
static int tablaVictorias[7][7] = {
    /* Piedra Papel Tijera Fuego Esponja Aire  Agua */
    /* Piedra  */ {  0,    0,    1,     1,    1,      0,    0 },
    /* Papel   */ {  1,    0,    0,     0,    1,      1,    0 },
    /* Tijera  */ {  0,    1,    0,     0,    1,      1,    0 },
    /* Fuego   */ {  0,    1,    1,     0,    1,      0,    0 },
    /* Esponja */ {  0,    0,    0,     0,    0,      0,    1 },
    /* Aire    */ {  1,    0,    0,     1,    0,      0,    0 },
    /* Agua    */ {  1,    1,    0,     1,    0,      0,    0 }
};
...
void descargarImagenesPartida() {
    for (int i = 0; i < 7; i++) { // Cambiado a 7
...
int actualizarPartida() {
    if (jugadaJugador == -1) {
        ...
        if (IsKeyPressed(KEY_SEVEN)) jugadaJugador = 6;

        if (jugadaJugador != -1) {
            jugadaPC = GetRandomValue(0, 6);
...
void dibujarPartida(int pantallaAncho) {
    if (jugadaJugador == -1) {
        ...
        for (int i = 0; i < 7; i++) {
            ...
            DrawText(linea, (pantallaAncho/2) - (ancho/2), 170 + (i * 45), 35, GRAY);
        }
    }
    else {
